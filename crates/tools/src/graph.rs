...
use emry_context::types::{GraphEdge, GraphSubgraph};
use anyhow::{anyhow, Result};

use std::collections::{HashSet, VecDeque}; // Added imports
use std::sync::Arc;

use emry_core::models::paths::Path; // Re-use core Path type
...
pub struct GraphResult {
    ...
    pub paths: Vec<Path>, // Using emry_core::models::paths::Path
}
    ...
    pub fn graph(
        ...
        match direction {
            GraphDirection::Out => {
                ...
                // Convert from emry_graph type to emry_context type
                ...
                if let Some(node) = graph.get_node(&node_id)? {
                     subgraph.nodes.push(emry_graph::graph::GraphNode {
                        id: node.id,
                        kind: node.kind,
                        label: node.label,
