use anyhow::Result;
use emry_core::models::{Chunk, ScoredChunk};
use emry_core::ranking::RankConfig;
use emry_core::traits::Embedder;
use emry_graph::graph::{CodeGraph, GraphNode};
use emry_graph::path::{PathBuilder, PathBuilderConfig};
...
use emry_store::file_blob_store::FileBlobStore;
...
impl IndexManager {
    ...
    fn chunk_from_store(&self, chunk_id: &str) -> Result<Option<Chunk>> {
        ...
        let language = meta
            ...
            .extension()
            .and_then(|e| e.to_str())
            .map(emry_core::models::Language::from_extension)
            .unwrap_or(emry_core::models::Language::Unknown);
        Ok(Some(Chunk {
            id: stored.id,
            language,
            file_path: meta.path,
    ...
    pub async fn search_contextual(
        ...
        limit: usize,
        rank_cfg: Option<RankConfig>,
    ) -> Result<emry_core::models::ContextualResult> {
        let cfg = rank_cfg.unwrap_or_default();
            ...
            for seed in seeds {
                if let Ok(p) = builder.find_paths_to(&seed, &[], &path_cfg) {
                    ...
                    for gp in p {
                        ...
                        paths.push(emry_core::models::paths::Path {
                            nodes,
                            edges,
        ...
        Ok(emry_core::models::ContextualResult {
            chunks,
            paths,
        })
    }
...
fn describe_path(path: &emry_graph::path::Path) -> Vec<String> {
